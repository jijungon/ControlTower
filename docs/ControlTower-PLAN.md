...
| 서버 | IP | user | 접속 키 | dbsafe | ncloud | 접속 명령(미리보기) |
|---|---|---|---|---|---|---|
| web-01 | 10.89.2.67 | deploy | `gw_prod_ed25519` | — | ✓ | `ssh -i gw_prod_ed25519 deploy@10.89.2.67` |
| db-01 | 10.89.3.10 | dbadmin | `db_ed25519` | ✓ | — | (dbsafe 경유) |
| build-01 | 144.24.73.187 | ci | `build_ed25519` | — | — | `ssh -i build_ed25519 ci@144.24.73.187` |

- `~/.ssh/config` 임포트로 채움(Host·HostName·User·IdentityFile). **경유 gw 열은 생략** — 대부분 서버 자체가 gw이거나 직접 접속이라 불필요.
- **dbsafe**: DB 접근제어(dbsafe) 경유가 필요한 서버 여부. **ncloud**: ncloud 등 클라우드 콘솔/배스천 경유가 필요한 서버 여부. → **어떻게 접속해야 하는지 판단**용 플래그.
- 표시는 키 **파일명·지문**만(무저장). "이 키 쓰는 서버?" 역방향 조회 지원.
